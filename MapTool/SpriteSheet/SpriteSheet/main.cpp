...
const std::string window_title_g = "SpriteSheet Demo";
const unsigned int window_width_g = 600;
const unsigned int window_height_g = 600;
const glm::vec3 viewport_background_color_g(0.0, 0.0, 0.2);

int totalNumberAnims = 4;
int animNumber = 1;	//First Anim = 1;
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods){
	...
	if (key == GLFW_KEY_SPACE && action == GLFW_PRESS) {
		animNumber++;
		if (animNumber > totalNumberAnims * 2) {
			animNumber = 1;
		}
	}
}
// Global texture info
GLuint tex[3];
int width, height; // image height,width
...
//	tex = new GLuint[3];
	glGenTextures(6, tex);
	setthisTexture(tex[0], "FABULOUS.png");
	setthisTexture(tex[1], "orb.png");
	setthisTexture(tex[2], "saw.png");
	setthisTexture(tex[3], "black.png");
	setthisTexture(tex[4], "white.png");
	setthisTexture(tex[5], "red.png");
...
int main(void){
    ...
    try {
        ...
        while (!glfwWindowShouldClose(window.getWindow())){
			...
			//spritesheet.setTargetPos(glm::vec3(screenSpaceMouseX, screenSpaceMouseY, 0.0f));
